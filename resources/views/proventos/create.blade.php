...
@section('content')
    <form action="{{ route('proventos.store') }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col">
                        <label for="">Provento</label>
                        <input
                            type="text"
                            required
                            minlength="5"
                ...
                <div class="row mt-3">
                    ...
                    <div class="col">
                        ...
                        <select id="categoria_provento_id" required name="categoria_provento_id" class="form-control @error('categoria_provento_id') is-invalid @enderror">
                            <option selected disabled>Escolha</option>
                            @forelse ($categoriasProventos as $categoria)
                                <option value="{{ $categoria->id }}" {{ $categoria->id == old('categoria_provento_id') ? 'selected' : '' }}> {{ $categoria->nome }}</option>
                            @empty
                                <option disabled>Sem registros</option>
                            @endforelse
                        </select>
                        @error('categoria_provento_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                </div>
        ...
        <div class="justify-start">
            ...
            <a href="{{ route('proventos.index') }}" class="btn btn-sm btn-secondary">
                Voltar
            </a>
        </div>
    </form>
